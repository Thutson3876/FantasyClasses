...
public class Skill implements Iterable<Skill> {
	...
	public GuiItem asGuiItem(AbstractGUI currentInv) {
		...
		Player p = this.ability.getPlayer();
		List<GuiItem> items = new ArrayList<>();
		SkillTreeGUI skillTreeGui = new SkillTreeGUI(p, name + " Sub-Skills", this, currentInv, items);
		for(Skill s : this.next) {
			items.add(s.asGuiItem(skillTreeGui));
		}
		
		skillTreeGui.setItems(items);
		skillTreeGui.defaultFillGaps(Material.BLACK_STAINED_GLASS_PANE);
		guiItem.setLinkedInventory(skillTreeGui);
		return guiItem;
	}
