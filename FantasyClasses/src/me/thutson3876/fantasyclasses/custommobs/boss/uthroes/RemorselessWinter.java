...
public class RemorselessWinter implements MobAbility {
	...
	private final int durationInTicks = 4 * 20;
	private final int tickRate = 7;
	private final int radius = 3;
	
	private final int freezeAmt = 60;
	private final double dmg = 0.0;
	...
	private void spawnSnowStorm(Mob entity) {
		...
		new BukkitRunnable() {
			...
			public void run() {
				...
				world.playSound(entity.getLocation(), Sound.ITEM_ELYTRA_FLYING, 1.2f, 0.9f);
				world.spawnParticle(Particle.SNOWFLAKE, entity.getLocation(), 40);
				/*for(Location l : sphere) {
					world.spawnParticle(Particle.SNOWFLAKE, l, 10);
				}*/
				for(Entity e : world.getNearbyEntities(entity.getLocation(), radius, radius, radius)){
